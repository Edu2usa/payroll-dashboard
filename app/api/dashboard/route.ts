...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: allPeriodsData, error: periodsError } = await supabaseServer
      ...
      .select(
        'id, period_start, period_end, check_date, total_earnings, total_net_pay, total_persons, total_hours, total_withholdings, total_deductions',
      )
    ...
    const periodChange = {
      earnings_change_pct: previousPeriod
        ? safePctChange(
            latestPeriod.total_earnings,
            previousPeriod.total_earnings,
          )
        : 0,
      net_pay_change_pct: previousPeriod
        ? safePctChange(
            latestPeriod.total_net_pay,
            previousPeriod.total_net_pay,
          )
        : 0,
      ...
      employees_change_pct: previousPeriod
        ? safePctChange(
            latestPeriod.total_persons,
            previousPeriod.total_persons,
          )
        : 0,
      withholdings_change_pct: previousPeriod
        ? safePctChange(
            latestPeriod.total_withholdings,
            previousPeriod.total_withholdings,
          )
        : 0,
    }
    ...
    if (topEarnersRows.length > 0) {
      ...
      const nameMap =
        employeeNameRows.length > 0
          ? Object.fromEntries(
              employeeNameRows.map((e: any) => [
                e.id,
    ...
    const overtimeSummary = {
      total_ot_hours: overtimeRows.reduce(
        (sum: number, e: any) => sum + (e.overtime_hours || 0),
        0,
      ),
      total_ot_earnings: overtimeRows.reduce(
        (sum: number, e: any) => sum + (e.overtime_earnings || 0),
        0,
      ),
      employees_with_ot: new Set(overtimeRows.map((e: any) => e.employee_id))
        .size,
    }
